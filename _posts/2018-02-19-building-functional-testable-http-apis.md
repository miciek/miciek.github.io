...
It contains the current state of a `PacMan`, next chosen direction, the grid set-up and remaining dot cells, which are cells that still contain a dot (the thing Pac-Man eats). 
...
We have defined several `routes` using [Akka HTTP](https://doc.akka.io/docs/akka-http/current/index.html) Route DSL which is completely fine. What's not fine is how much does this HTTP module know about other concerns in the application, namely `var state`, `GameEngine` and `GameState`.
...
Finally, the whole thing is totally untestable. HTTP & JSON should have their own unit tests that check whether the API is properly configured. These particular tests shouldn't test any game logic, which should be tested separately in `GameEngine` unit tests. 

All these issues tend to get worse and worse as project gets bigger.[^tarpit] That's why we need to take serious measures to make it right in the early days, when it's still easy. In this post I will show you how to disentangle the concerns in our simple application using the following tools: **functions as parameters** and **type parameters**.

## Abstracting over state using type parameters
First change I always try to do when I feel one of my modules deals with too many concerns is to abstract over as many concrete types as I can. This simple exercise provides me with enough insight about the design to make the rest of the refactoring steps more natural. 
...
Now the compiler will tell us what parts of the code were using `GameState` and we need to investigate each of those problems separately by asking a question: *"Does it really need to know?"*. In our case: *"Does `HttpRoutes` really need to know about `GameState`?"*. Let's find out by trying to deal with compile errors.
...
### But... why?
Why do we want to do that knowing that `G` will always be a `GameState`? This is a very simple way to tell that this piece of code is independent of `GameState`. It only depends on `PacMan` and needs a way to get it from a `G`. It cannot create an instance of `G` or modify `G`, because it doesn't know what `G` is. The code is **constrained** which makes it far easier to comprehend[^dijkstra]. On the other hand, it gives much more **liberty** to the modules that use it, because they can choose what `G` is and how to get a `PacMan` from `G`.[^runar]

## Using more functions as parameters
We got rid of the dependency to `GameState` in one place, but we can do even better. Let's think about the responsibility of our `POST /games` endpoint. 

This is how the code that handles game creation looks like:
```scala
val startedGame: Either[String, GameState] = GameEngine.start(request.gridName, Grid.fromName)
startedGame match {
    case Right(game) =>
      val gameId = state.size
      state = state + (gameId -> game)
...
It gets a `gridName` string as an input and completes the request returning `gameId` integer or a failure message. TIt use `GameEngine.start` directly so it "knows" everything about it. However, when you think about it, this endpoint doesn't really need to know about `GameEngine`! And we can use the same technique to figure it out: replace direct call with a more generic function, which is passed as parameter. Let's see how it works in this case.
...
The first statement can be encoded as a function `createGame: String => Either[String, G]`. The second statement can be encoded `addNewGame: G => Int`. No `GameState`, no `GameEngine`. Just as much information as this layer really needs, but no more. Truly separated concerns.
...
There is another quick win we've got after doing the above refactorings. We can test our HTTP layer in total isolation! That means we can have *unit tests* for our endpoints without worrying about state or game logic. For example let's see the test for `getGameRoute` function:
...
In this post I have shown you how we can separate concerns of a simple web application. We have separated:
 ...
 - *state management* (`var state: Map`),
 - *game logic handling* (`GameEngine`) and
 - *HTTP handling* (`HttpRoutes`).
